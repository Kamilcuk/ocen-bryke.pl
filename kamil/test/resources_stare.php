<?php
// versja 0.2
// dodalem mysql-escape-query
// user->edytuj mozna edytowac dowolna kolumne
// poprawiano db->query - nie wykrzacza na insertach
// nazwy obiektów takie jak nazwy tablel
$settings = array (
		'host' => '127.0.0.1',
		'user' => 'projekt',
		'pass' => 'projekt123',
...
class Uzytkownik {
	...
	/*function edytuj($wiek, $email, $ranga, $admin) {
		...
		foreach(array_combine($names, $values) as $name => $value) {
			$str = $str.' `'.$name.'`='.static::$db->escape($value).',';
		}
		rtrim($str, ',');
		static::$db->query('UPDATE `'.$this->table.'` WHERE nick = \''.$this->nick.'\' SET '.$str);
	}*/
